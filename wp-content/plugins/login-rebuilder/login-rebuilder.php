...
/*
 Copyright (C) 2013-2016 tmatsuur (Email: takenori dot matsuura at 12net dot jp)
...
define( 'LOGIN_REBUILDER_DB_VERSION', '2.1.0' );
...
?>";
	...
	var $remote_addr;	// maybe $_SERVER['REMOTE_ADDR']
	var $request_uri; // maybe $_SERVER['REQUEST_URI']
	...
	public function __construct() {
		...
		$this->remote_addr = preg_match( '/^(([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]).){3}([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/', $remote_addr )? $remote_addr: self::INVALID_REMOTE_ADDR;
		...
		if ( $this->properties['status'] == self::LOGIN_REBUILDER_STATUS_WORKING ) {
			...
			add_filter( 'authenticate', array( &$this, 'role_authenticate' ), 9199, 3 );
			if ( isset( $this->properties['ambiguous_error_message'] ) && $this->properties['ambiguous_error_message'] ) {	// [2.1.0]
				add_filter( 'authenticate', array( &$this, 'ambiguous_error_message' ), 9999, 3 );
			}
	...
	private function _update_logging( &$logging, $type = null ) {
		if ( !is_null( $type ) ) {
			if ( isset( $logging[$type] ) && is_array( $logging[$type] ) ) {
				if ( count( $logging[$type] ) >= self::LOGIN_REBUILDER_LOGGING_LIMIT ) array_shift( $logging[$type] );
			} else
				$logging[$type] = array();
		}
...
?>
<div id="invalid-log-title" ><?php _e( "Request datetime - Username(Requesting IP)", LOGIN_REBUILDER_DOMAIN ); ?></div>
...
<?php
	...
	private function _view_log_of_login( $log_login ) {
		...
		$users = array();
?>
<div id="login-log-title" ><?php _e( "Request datetime - Username(Requesting IP)", LOGIN_REBUILDER_DOMAIN ); ?></div>
...
<?php
		foreach ( $log_login as $log ) {
			...
			echo esc_html( date_i18n( $date_time_format, $log['time']+$gmt_offset*3600 ) ).' - '.esc_html( $_user->user_nicename.'('.$log['ip'].')' );
?><br />
<?php
		}
?>
...
?></p>
<div id="pingback-log-title" ><?php _e( "Request datetime - Requesting IP|Status|From Hostname|(Error)", LOGIN_REBUILDER_DOMAIN ); ?></div>
...
<?php
	 ...
	 * @param string       $username User login.
	 * @param string       $password User password
	 * @return null|WP_User|WP_Wrror User to authenticate.
	 */
